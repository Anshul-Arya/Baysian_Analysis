# Graph of normal probability density function, with comb interval
...
text(-sdval, 0.9*max(y), bquote(paste(mu , " = ", .(meanval))), adj = c(1,.5))
text(-sdval, 0.8*max(y), bquote(paste(sigma, " = ", .(sdval))), adj = c(1,.5))
text(sdval, 0.9*max(y), bquote(paste(Delta, " = ", .(dx))), adj = c(1,.5))
text( sdval , .8*max(y) ,
      bquote(
        paste( sum(,x,) , " " , Delta , "x p(x) = " , .(signif(area,3)) )
        ) , adj=c(0,.5) )
# Save the plot to pdf file
dev.copy2pdf(file = "IntegralofDensity.pdf")
